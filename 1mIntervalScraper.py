...
if datetime.today().strftime('%Y-%m-%d') not in closedDatesArray:
    # Begin scraping
    logName = datetime.today().strftime('C:/Users/tsher/Desktop/MarketMaker/1mIntervalScraper/log/%Y-%m-%d_minute.log')
    ...
    for ticker in s_and_p_500:
        ...
        try:
            data = yf.download(tickers=ticker.strip(), period=daysToCollectString, interval='1m')

            tableName = ticker.strip().lower() + '_minute'
